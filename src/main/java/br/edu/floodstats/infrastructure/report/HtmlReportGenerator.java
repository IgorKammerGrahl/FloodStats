...
public class HtmlReportGenerator implements ReportGenerator {
    ...
    public void generate(StatisticalResult result, List<HydroRecord> records, String outputPath) throws Exception {
        ...
        html.append("<section id=\"estatisticas\">\n<h2>Resultados Estatísticos</h2>\n<table>\n")
                ...
                .append(String.format("<tr><td>Média</td><td>%.2f %s</td></tr>%n", result.getMean(), unit))
                .append(String.format("<tr><td>Mediana</td><td>%.2f %s</td></tr>%n", result.getMedian(), unit))
                .append(String.format("<tr><td>Moda</td><td>%.2f %s</td></tr>%n", result.getMode(), unit))
                .append(String.format("<tr><td>Variância</td><td>%.2f</td></tr>%n", result.getVariance()))
                .append(String.format("<tr><td>Desvio Padrão</td><td>%.2f</td></tr>%n", result.getStandardDeviation()))
                .append(String.format("<tr><td>Tendência (%s/dia)</td><td>%.4f ( %s )</td></tr>%n", unit,
                        result.getTrendSlope(), result.getTrendDescription()))
                .append("</table>\n</section>\n");

        // Chart section
        html.append("<section id=\"grafico\">\n<h2>Gráfico Temporal</h2>\n")
                .append(buildSvgChart(sortedRecords, result.getTrendSlope(), result.getTrendIntercept()))
                .append("</section>\n");
        ...
        for (int i = 0; i < sampleLimit; i++) {
            ...
            html.append(String.format("<tr><td>%s</td><td>%.2f</td><td>%s</td></tr>%n", hydroRecord.getDate(),
    ...
    private String buildSvgChart(List<HydroRecord> records, double slope, double intercept) {
        ...
        svg.append(String.format("<svg width=\"%d\" height=\"%d\" xmlns=\"http://www.w3.org/2000/svg\">%n", svgWidth,
                svgHeight));
        ...
        if (numberOfRecords > 1 && (maxVal - minVal) > 0) {
            ...
            int startX = padding;
            int startY = (int) (svgHeight - padding - ((startVal - minVal) / (maxVal - minVal) * graphHeight));
            ...
            // Legend
            svg.append(String.format("<text x=\"%d\" y=\"%d\" font-size=\"14\" fill=\"#e74c3c\">Tendência</text>%n",
                    svgWidth - padding - 80, padding + 20));
            svg.append(String.format("<text x=\"%d\" y=\"%d\" font-size=\"14\" fill=\"#3498db\">Dados Reais</text>%n",
